...
Security in Rabbitai is handled by Flask AppBuilder (FAB), an application development framework
...
Rabbitai ships with a set of roles that are handled by Rabbitai itself. You can assume
that these roles will stay up-to-date as Rabbitai evolves (and as you update Rabbitai versions).
...
the **rabbitai init** command (often done between Rabbitai versions).
...
To allow logged-out users to access some Rabbitai features, you can use the `PUBLIC_ROLE_LIKE` config setting and assign it to another role whose permissions you want passed to this role.

For example, by setting `PUBLIC_ROLE_LIKE = Gamma` in your `rabbitai_config.py` file, you grant
...
On top of that, Rabbitai can expose more granular permissions like **all_datasource_access**.

**We do not recommend altering the 3 base roles as there are a set of assumptions that
Rabbitai is built upon**. It is possible though for you to create your own roles, and union them to existing ones.

### Permissions

Roles are composed of a set of permissions, and Rabbitai has many categories of
...
software projects. Apache Rabbitai is highly sensitive and forthcoming to issues pertaining to its
features and functionality.


If you have apprehensions regarding Rabbitai security or you discover vulnerability or potential
threat, don’t hesitate to get in touch with the Apache Security Team by dropping a mail at
security@apache.org. In the mail, specify the project name Rabbitai with the description of the
issue or potential threat. You are also urged to recommend the way to reproduce and replicate the
issue. The security team and the Rabbitai community will get back to you after assessing and
