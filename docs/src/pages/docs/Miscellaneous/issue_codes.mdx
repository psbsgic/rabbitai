...
Rabbitai and provides additional context.
...
issue within Rabbitai, but instead a problem with the underlying
...
```
Rabbitai encountered an error while running a command.
```

Something unexpected happened, and Rabbitai encountered an error while
running a command. Please reach out to your administrator.

## Issue 1011

```
Rabbitai encountered an unexpected error.
```

Something unexpected happened in the Rabbitai backend. Please reach out
...
Your instance of Rabbitai doesn't have a results backend configured, which is needed for asynchronous queries. Please contact an administrator for further assistance.
...
The submitted query might be too complex to run under the time limit defined by your Rabbitai administrator. Please double check your query and verify if it can be optimized. Alternatively, contact your administrator to increase the timeout period.
