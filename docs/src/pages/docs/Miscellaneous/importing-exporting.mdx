...
The rabbitai cli allows you to import and export datasources from and to YAML. Datasources include
...
```
rabbitai export_datasources
```

To save your datasources to a file run:

```
rabbitai export_datasources -f <filename>
```
...
```
rabbitai export_datasource_schema
```
...
```
rabbitai import_datasources -p <path or filename>
```
...
```
rabbitai import_datasources -p <path> -r
```
...
```
rabbitai import_datasources -p <path / filename> -s columns,metrics
```

This will sync all metrics and columns for all datasources found in the `<path /filename>` in the
Rabbitai meta database. This means columns and metrics not specified in YAML will be deleted. If you
