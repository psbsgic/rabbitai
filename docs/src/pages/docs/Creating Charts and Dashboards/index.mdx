...
This section is focused on documentation for end-users who will be using Rabbitai
...
This tutorial targets someone who wants to create charts and dashboards in Rabbitai. We’ll show you
how to connect Rabbitai to a new database and configure a table in that database for analysis.
...
Rabbitai itself doesn't have a storage layer to store your data but instead pairs with
...
to query and visualize data from it. If you're using Rabbitai locally via
[Docker compose](/docs/installation/installing-rabbitai-using-docker-compose), you can
skip this step because a Postgres database, named **examples**, is included and
pre-configured in Rabbitai for you.
...
Congratulations, you've just added a new data source in Rabbitai!

### Registering a new table

Now that you’ve configured a data source, you can select specific tables (called **Datasets** in Rabbitai)
that you want exposed in Rabbitai for querying.
...
- If it's a datetime column, how should Rabbitai parse
...
### Rabbitai semantic layer

Rabbitai has a thin semantic layer that adds many quality of life improvements for analysts.
The Rabbitai semantic layer can store 2 types of computed data:
...
Rabbitai has 2 main interfaces for exploring data:
...
In the following screenshot, we save the chart to a new "Rabbitai Duper Sales Dashboard":

<img src="/images/tutorial_save_slice.png" />

To publish, click **Save and goto Dashboard**.

Behind the scenes, Rabbitai will create a slice and store all the information needed
...
Congrats! You’ve successfully linked, analyzed, and visualized data in Rabbitai. There are a wealth
