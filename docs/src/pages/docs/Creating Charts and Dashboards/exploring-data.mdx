---
name: Exploring Data in Rabbitai
...
## Exploring Data in Rabbitai

In this tutorial, we will introduce key concepts in Apache Rabbitai through the exploration of a
...
[Github](https://raw.githubusercontent.com/apache-rabbitai/examples-data/master/tutorial_flights.csv).
In the Rabbitai menu, select **Data ‣ Upload a CSV**.
...
By default, Apache Rabbitai only shows the last week of data. In our example, we want to visualize all
...
SUM aggregate. Note that Apache Rabbitai will indicate the type of the metric by the symbol on the
...
- Set the Annotation Source as Rabbitai annotation
...
In this section, we are going to explore the Advanced Analytics feature of Apache Rabbitai that
