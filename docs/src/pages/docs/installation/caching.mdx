...
Rabbitai uses [Flask-Caching](https://flask-caching.readthedocs.io/) for caching purpose. For security reasons,
there are two separate cache configs for Rabbitai's own metadata (`CACHE_CONFIG`) and charting data queried from
...
`rabbitai_config.py` that complies with [the Flask-Caching specifications](https://flask-caching.readthedocs.io/en/latest/#configuring-flask-caching).
...
Rabbitai has a Celery task that will periodically warm up the cache based on different strategies.
...
strategies, check the `rabbitai/tasks/cache.py` file.
...
Using the above example cache keys for dashboards will be `rabbitai_thumb__dashboard__{ID}`. You can
