...
### Install Yeoman and the Rabbitai Package Generator
...
[rabbitai package generator](https://github.com/apache-rabbitai/rabbitai-ui/tree/master/packages/generator-rabbitai)
(`v0.14.7`) to create the new plugin.

```
npm install -g yo @rabbitai-ui/generator-rabbitai
```

### Install Rabbitai

There are
[complete instructions](https://github.com/apache/rabbitai#installation-and-configuration)
available on the [Rabbitai Github repository](https://github.com/apache/rabbitai). In a
nutshell, the easiest way is to:

1. Have a Mac or linux-based machine
2. Install [Docker](https://docs.docker.com/get-docker/)
3. Clone [the repository](https://github.com/apache/rabbitai) to your computer
4. Use your terminal to `cd` into the `rabbitai` directory
5. Run `docker-compose up`
6. Open _another_ terminal, and `cd` into `rabbitai/rabbitai-frontend`
...
1. Clone [the `rabbitai-ui` repository](https://github.com/apache-rabbitai/rabbitai-ui) to your
   computer. It can sit in the same parent directory as your `rabbitai` repo
2. Use your terminal to `cd` into `rabbitai-ui`
...
1. Using your terminal, `cd` into your local `rabbitai-ui` repo folder and then into the `plugins`
   subdirectory.
...
4. Finally, run `yo @rabbitai-ui/rabbitai`
5. Select `Create rabbitai-ui chart plugin package` on the following screen:
...
10. Select whether or not you want to include badges at the top of your README file (really only
    needed if you intend to contribute your plugin to the `rabbitai-ui` repo).
...
### Add your Plugin to Rabbitai (with NPM Link)

Now, we want to see this thing actually RUN! To do that, we'll add your package to Rabbitai and
embrace the magic power of `npm link` to see it in-situ, without needing to **build** the plugin, or
open any PRs on Github.

1. Add your package to the `package.json` file in `rabbitai/rabbitai-frontend`.
...
2. Add your plugin to the `MainPreset.js` file (located in
   `rabbitai/rabbitai-frontend/src/visualizations/presets/MainPreset.js`) in two places,
...
3. Open a terminal window to `rabbitai/rabbitai-frontend`. If you did the Install Rabbitai
...
4) Use `npm link` to symlink plugin, using a relative path to `rabbitai-ui` and your plugin folder,
   e.g. `npm link ../../rabbitai-ui/plugins/plugin-chart-hello-world`.

5. Restart your webpack dev server with `npm run dev-server`. You'll know it worked if you see a
   line stating
   `[Rabbitai Plugin] Use symlink source for @rabbitai-ui/plugin-chart-hello-world @ ^0.0.0`.
...
You should now be able to go to the Explore view in your local Rabbitai and add a new chart! You'll
...
looking at existing `rabbitai-ui` plugins for more examples of how you can implement controls to
enhance your queries, work with your data, and change your visualization's display.

### Build the perfect query

The `buildQuery` file where your plugin actually fetches data from the Rabbitai backend. This file
...
This file also allows you to add various post-processing operations, to have the Rabbitai backend
...
has access to Rabbitai's burgeoning set of theme variables, and also automatically scopes the styles
to your plugin, so they don't "leak" to other areas of Rabbitai.
...
continue to push Rabbitai (and the viz plugins) toward the standards of the Rabbitai redesign (see
[SIP-34](https://github.com/apache/rabbitai/issues/8976))
...
In this tutorial, you built your plugin in the `rabbitai-ui` repo. This means you can use the
built-in build scripts that the repo provides. With your terminal of choice, simply `cd` into the
root directory of `supeset-ui` and run `yarn build`. This will kick off a build of ALL the Rabbitai
...
`rabbitai-ui` will run all the tests for plugins/packages, including your Hello World.

### Deploying Custom Visualization to Production

To deploy plugins to a production environment, you must have additional code
inside Rabbitai that includes the npm packages of your plugins so they can be installed in the frontend.
