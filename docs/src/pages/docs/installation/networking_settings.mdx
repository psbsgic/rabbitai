...
[PR 5039](https://github.com/apache/rabbitai/pull/5039) adds domain sharding to Rabbitai,
and this feature will be enabled by configuration only (by default Rabbitai doesn’t allow
cross-domain request).

Add the following setting in your `rabbitai_config.py` file:

- `RABBITAI_WEBSERVER_DOMAINS`: list of allowed hostnames for domain shareding feature.

### Middleware

Rabbitai allows you to add your own middleware. To add your own middleware, update the
`ADDITIONAL_MIDDLEWARE` key in your `rabbitai_config.py`. `ADDITIONAL_MIDDLEWARE` should be a list
