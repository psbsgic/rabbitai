...
In this section, we'll walkthrough the  pre-defined Jinja macros in Rabbitai.

**Current Username**

The `{{ current_username() }}` macro returns the username of the currently logged in user.

If you have caching enabled in your Rabbitai configuration, then by defaul the the `username` value will be used
by Rabbitai when calculating the cache key. A cache key is a unique identifer that determines if there's a
cache hit in the future and Rabbitai can retrieve cached data.
...
If you have caching enabled in your Rabbitai configuration, then by defaul the the `user_id` value will be used
by Rabbitai when calculating the cache key. A cache key is a unique identifer that determines if there's a
cache hit in the future and Rabbitai can retrieve cached data.
...
- You're hosting Rabbitai at the domain www.example.com and you send your
coworker in Spain the following SQL Lab URL `www.example.com/rabbitai/sqllab?countrycode=ES`
and your coworker in the USA the following SQL Lab URL `www.example.com/rabbitai/sqllab?countrycode=US`
...
The `{{ cache_key_wrapper() }}` function explicitly instructs Rabbitai to add a value to the
...
[here](https://github.com/apache/rabbitai/blob/efd70077014cbed62e493372d33a2af5237eaadf/rabbitai/jinja_context.py#L133-L148).
