...
Running on Kubernetes is supported with the provided [Helm](https://helm.sh/) chart found in the official [Rabbitai helm repository](https://apache.github.io/rabbitai/index.yaml).
...
1. Add the Rabbitai helm repository

```sh
helm repo add rabbitai https://apache.github.io/rabbitai
"rabbitai" has been added to your repositories
```

1. View charts in repo
```sh
helm search repo rabbitai
NAME                    CHART VERSION   APP VERSION     DESCRIPTION
rabbitai/rabbitai       0.1.1           1.0             Apache Rabbitai is a modern, enterprise-ready b...
```

1. Configure your setting overrides

Just like any typical Helm chart, you'll need to craft a `values.yaml` file that would define/override any of the values exposed into the default [values.yaml](https://github.com/apache/rabbitai/tree/master/helm/rabbitai/values.yaml), or from any of the dependent charts it depends on:
...
```sh
helm upgrade --install --values my-values.yaml rabbitai rabbitai/rabbitai
```
...
rabbitai-celerybeat-7cdcc9575f-k6xmc    1/1     Running     0          119s
rabbitai-f5c9c667-dw9lp                 1/1     Running     0          4m7s
rabbitai-f5c9c667-fk8bk                 1/1     Running     0          4m11s
rabbitai-init-db-zlm9z                  0/1     Completed   0          111s
rabbitai-postgresql-0                   1/1     Running     0          6d20h
rabbitai-redis-master-0                 1/1     Running     0          6d20h
rabbitai-worker-75b48bbcc-jmmjr         1/1     Running     0          4m8s
rabbitai-worker-75b48bbcc-qrq49         1/1     Running     0          4m12s
```

The exact list will depend on some of your specific configuration overrides but you should generally expect:

* N `rabbitai-xxxx-yyyy` and `rabbitai-worker-xxxx-yyyy` pods (depending on your `replicaCount` value)
* 1 `rabbitai-postgresql-0` depending on your postgres settings
* 1 `rabbitai-redis-master-0` depending on your redis settings
* 1 `rabbitai-celerybeat-xxxx-yyyy` pod if you have `rabbitaiCeleryBeat.enabled = true` in your values overrides

1. Access it

The chart will publish appropriate services to expose the Rabbitai UI internally within your k8s cluster. To access it externally you will have to either:
...
* Run `kubectl port-forward rabbitai-xxxx-yyyy :8088` to directly tunnel one pod's port into your localhost
...
The default `rabbitai_config.py` is fairly minimal and you will very likely need to extend it. This is done by specifying one or more key/value entries in `configOverrides`, e.g.:
...
The entire `rabbitai_config.py` will be installed as a secret, so it is safe to pass sensitive parameters directly... however it might be more readable to use secret env variables for that.
...
Those can be passed as key/values either with `extraEnv` or `extraSecretEnv` if they're sensitive. They can then be referenced from `rabbitai_config.py` using e.g. `os.environ.get("VAR")`.
...
configOverrides:
  smtp: |
    ...
    SMTP_USER = os.getenv("SMTP_USER","rabbitai")
    SMTP_PORT = os.getenv("SMTP_PORT",25)
    SMTP_PASSWORD = os.getenv("SMTP_PASSWORD","rabbitai")
```
...
configOverrides:
  enable_oauth: |
    ...
    # Map Authlib roles to rabbitai roles
...
rabbitaiWorker:
  command:
    ...
    - |
      ...
      # See https://github.com/apache/rabbitai/blob/4fa3b6c7185629b87c27fc2c0e5435d458f7b73d/docs/src/pages/docs/installation/email_reports.mdx
      apt update
...
configOverrides:
  feature_flags: |
    ...
    SMTP_PORT = os.getenv("SMTP_PORT",25)
    SMTP_PASSWORD = os.getenv("SMTP_PASSWORD","rabbitai")
    SMTP_MAIL_FROM = os.getenv("SMTP_MAIL_FROM","rabbitai@rabbitai.com")
  ...
  reports: |
    ...
    WEBDRIVER_BASEURL = "http://{{ template "rabbitai.fullname" . }}:{{ .Values.service.port }}/"
