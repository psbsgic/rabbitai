...
route: /docs/installation/installing-rabbitai-from-scratch
index: 2
version: 1
---

## Installing Rabbitai from Scratch

### OS Dependencies

Rabbitai stores database connection information in its metadata database. For that purpose, we use
...
You should install a recent version of Python (Rabbitai uses 3.7.9). We'd recommend using a Python version manager like [pyenv](https://github.com/pyenv/pyenv) (and also [pyenv-virtualenv](https://github.com/pyenv/pyenv-virtualenv)).
...
We highly recommend installing Rabbitai inside of a virtual environment. Python ships with
...
```
# Here we name the virtual env 'rabbitai'
pyenv virtualenv rabbitai
pyenv activate rabbitai
```
...
### Installing and Initializing Rabbitai

First, start by installing `apache-rabbitai`:
...
### Installing Rabbitai with Helm in Kubernetes

You can install Rabbitai into Kubernetes with [Helm](https://helm.sh/). The chart is located in
the `helm/` directory.

To install Rabbitai in your Kubernetes cluster with Helm 3, run:

```
helm dep up ./helm/rabbitai
helm upgrade --install rabbitai ./helm/rabbitai
```

Note that the above command will install Rabbitai into `default` namespace of your Kubernetes
cluster.
