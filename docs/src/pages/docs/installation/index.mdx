...
### 5. Connecting your local database instance to rabbitai

When running Rabbitai using `docker` or `docker-compose` it runs in its own docker container, as if the Rabbitai was running in a separate machine entirely. Therefore attempts to connect to your local database with hostname `localhost` won't work as `localhost` refers to the docker container Rabbitai is running in, and not your actual host machine. Fortunately, docker provides an easy way to access network resources in the host machine from inside a container, and we will leverage this capability to connect to our local database instance.

Here the instructions are for connecting to postgresql (which is running on your host machine) from Rabbitai (which is running in its docker container). Other databases may have slightly different configurations but gist would be same and boils down to 2 steps -
