...
Rabbitai by default logs special action events in its internal database. These logs can be accessed
...
Rabbitai can be instrumented to log events to StatsD if desired. Most endpoints hit are logged as
well as key events like query start and end in SQL Lab.

To setup StatsD logging, it’s a matter of configuring the logger in your `rabbitai_config.py`.
