...
Not in the Explore or Visualization UI. A Rabbitai SQLAlchemy datasource can only be a single table
or a view.
...
expressions. The limitation there is your database performance as Rabbitai effectively will run a
query on top of your query (view). A good practice may be to limit yourself to joining your main
large table to one or many small tables only, and avoid using _GROUP BY_ where possible as Rabbitai
...
serve it in an interactive fashion to provide a good user experience in Rabbitai.

### How BIG can my datasource be?

It can be gigantic! Rabbitai acts as a thin layer above your underlying databases or data engines.
...
For running long query from Sql Lab, by default Rabbitai allows it run as long as 6 hours before it
...
Rabbitai is running on gunicorn web server, which may time out web requests. If you want to increase
the default (50), you can specify the timeout when starting the web server with the -t flag, which
is expressed in seconds.

```
rabbitai runserver -t 300
```
...
from Rabbitai server (which is processing long queries), these web servers will send 504 status code
to clients directly. Rabbitai has a client-side timeout limit to address this issue. If query didn’t
come back within client-side timeout (60 seconds by default), Rabbitai will display warning message
to avoid gateway timeout message. If you have a longer gateway timeout limit, you can change the
timeout settings in **rabbitai_config.py**:
...
to **rabbitai_config.py** at the key MAPBOX_API_KEY:
...
**Why does ‘flask wab’ or rabbitai freezed/hung/not responding when started (my home directory is
NFS mounted)?**

By default, Rabbitai creates and uses an SQLite database at `~/.rabbitai/rabbitai.db`. SQLite is
...
You can override this path using the **RABBITAI_HOME** environment variable.

Another workaround is to change where rabbitai stores the sqlite database by adding the following in
`rabbitai_config.py`:

```
SQLALCHEMY_DATABASE_URI = 'sqlite:////new/location/rabbitai.db'
```

You can read more about customizing Rabbitai using the configuration file
[here](/docs/installation/configuring-rabbitai).

### What if the table schema changed?

Table schemas evolve, and Rabbitai needs to reflect that. It’s pretty common in the life cycle of a
dashboard to want to add a new dimension or metric. To get Rabbitai to discover your new columns,
...
### What database engine can I use as a backend for Rabbitai?

To clarify, the database backend is an OLTP database used by Rabbitai to store its internal
information like your list of users, slices and dashboard definitions.

Rabbitai is tested using Mysql, Postgresql and Sqlite for its backend. It’s recommended you install
Rabbitai on one of these database server for production.
...
fine as a datasource for Rabbitai, just not as the OLTP backend.
...
### How do I get Rabbitai to refresh the schema of my table?

When adding columns to a table, you can have Rabbitai detect and merge the new columns in by using
...
### Does Rabbitai work with [insert database engine here]?
...
For a database engine to be supported in Rabbitai through the SQLAlchemy connector, it requires
...
database to be supported is through the Rabbitai Database Engine Specification interface. This
...
- date-related SQL function that allow Rabbitai to fetch different time granularities when running
  time-series queries
- whether the engine supports subqueries. If false, Rabbitai may run 2-phase queries to compensate
  for the limitation
...
Beyond the SQLAlchemy connector, it’s also possible, though much more involved, to extend Rabbitai
...
### Does Rabbitai offer a public API?
...
[SIP-17](https://github.com/apache/rabbitai/issues/7259) and constant progress has been
...
made available under **/swagger/v1** by enabling the following flag in `rabbitai_config.py`:

```
FAB_API_SWAGGER_UI = True
```

There are other undocumented [private] ways to interact with Rabbitai programmatically that offer no
guarantees and are not recommended but may fit your use case temporarily:

- using the ORM (SQLAlchemy) directly
- using the internal FAB ModelView API (to be deprecated in Rabbitai)
