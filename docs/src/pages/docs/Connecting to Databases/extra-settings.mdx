...
**database**. For Rabbitai to connect to a specific schema, you can set the **schema** parameter in
the **Edit Tables** form (Sources > Tables > Edit record).

### External Password Store for SQLAlchemy Connections

Rabbitai can be configured to use an external store for database passwords. This is useful if you a
running a custom secret distribution framework and do not wish to store secrets in Rabbitai’s meta
database.
