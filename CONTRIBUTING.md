...
Here's a list of repositories that contain Rabbitai-related packages:

- [apache/rabbitai](https://github.com/apache/rabbitai)
  is the main repository containing the `apache-rabbitai` Python package
  distributed on
  [pypi](https://pypi.org/project/apache-rabbitai/). This repository
  also includes Rabbitai's main TypeScript/JavaScript bundles and react apps under
  the [rabbitai-frontend](https://github.com/apache/rabbitai/tree/master/rabbitai-frontend)
  folder.
- [apache-rabbitai/superset-ui](https://github.com/apache-rabbitai/superset-ui)
  contains core Rabbitai's
  [npm packages](https://github.com/apache-rabbitai/superset-ui/tree/master/packages).
  These packages are shared across the React apps in the main repository,
  and in visualization plugins.
- [apache-rabbitai/superset-ui-plugins](https://github.com/apache-rabbitai/superset-ui-plugins)
  contains the code for the default visualizations that ship with Rabbitai
  and are maintained by the core community.
- [apache-rabbitai/superset-ui-plugins-deckgl](https://github.com/apache-rabbitai/superset-ui-plugins-deckgl)
  contains the code for the geospatial visualizations that ship with Rabbitai
  and are maintained by the core community.
- [github.com/apache-rabbitai](https://github.com/apache-rabbitai) is the
  Github organization under which we manage Rabbitai-related
  small tools, forks and Rabbitai-related experimental ideas.
...
For large features or major changes to codebase, please create **Rabbitai Improvement Proposal (SIP)**. See template from [SIP-0](https://github.com/apache/rabbitai/issues/5602)
...
Rabbitai could always use better documentation,
whether as part of the official Rabbitai docs,
...
text strings from Rabbitai's UI. You can jump in to the existing
language dictionaries at
`rabbitai/translations/<language_code>/LC_MESSAGES/messages.po`, or
even create a dictionary for a new language altogether.
See [Translating](#translating) for more details.

### Ask Questions

There is a dedicated [`apache-rabbitai` tag](https://stackoverflow.com/questions/tagged/apache-rabbitai) on [StackOverflow](https://stackoverflow.com/). Please use it when asking questions.
...
**Feature/Large changes:** If you intend to change the public API, or make any non-trivial changes to the implementation, we require you to file a new issue as `#SIP` (Rabbitai Improvement Proposal). This lets us reach an agreement on your proposal before you put significant effort into it. You are welcome to submit a PR along with the SIP (sometimes necessary for demonstration), but we will not review/merge the code until the SIP is approved.
...
| `#SIP`          | Rabbitai Improvement Proposal                                                                                                           | N/A                                                                                                                                               |
...
When creating a PR, if you're aiming to have it included in a specific release, please tag it with the version label. For example, to have a PR considered for inclusion in Rabbitai 1.1 use the label `v1.1`.

## Reporting a Security Vulnerability

Please report security vulnerabilities to private@rabbitai.apache.org.

In the event a community member discovers a security flaw in Rabbitai, it is important to follow the [Apache Security Guidelines](https://www.apache.org/security/committers.html) and release a fix as quickly as possible before public disclosure. Reporting security vulnerabilities through the usual GitHub Issues channel is not ideal as it will publicize the flaw before a fix can be applied.
...
```bash
git clone git@github.com:your-username/rabbitai.git
cd rabbitai
```

### Documentation

The latest documentation and tutorial are available at https://rabbitai.apache.org/.
...
images (and any other static assets) to the `rabbitai-frontend/images` directory.
When the docs are deployed to https://rabbitai.apache.org/, images
are copied from there to the `_static/images` directory, just like they're referenced
in the docs.

For example, the image referenced above actually lives in `rabbitai-frontend/images/tutorial`. Since the image is moved during the documentation build process, the docs reference the image in `_static/images/tutorial` instead.

### Flask server

#### OS Dependencies

Make sure your machine meets the [OS dependencies](https://rabbitai.apache.org/docs/installation/installing-rabbitai-from-scratch#os-dependencies) before following these steps.
...
FLASK_ENV=development rabbitai run -p 8088 --with-threads --reload --debugger
...
# Install rabbitai pip packages and setup env only
$ make rabbitai
...
```bash
FLASK_ENV=development gunicorn "rabbitai.app:create_app()" -k "geventwebsocket.gunicorn.workers.GeventWebSocketWorker" -b 127.0.0.1:8088 --reload
```
...
For those interested, you may also try out [avn](https://github.com/nvm-sh/nvm#deeper-shell-integration) to automatically switch to the node version that is required to run Rabbitai frontend.
...
npm run dev-server -- --rabbitaiPort=8081

# Proxy to a remote backend but serve local assets
npm run dev-server -- --rabbitai=https://rabbitai-dev.example.com
```

The `--rabbitai=` option is useful in case you want to debug a production issue or have to setup Rabbitai behind a firewall. It allows you to run Flask server in another environment while keep assets building locally for the best developer experience.
...
`rabbitai-frontend/package-lock.json` is updated according to `npm`-prescribed
best practices.

#### Feature flags

Rabbitai supports a server-wide feature flag system, which eases the incremental development of features. To add a new feature flag, simply modify `rabbitai_config.py` with something like the following:
...
If you want to use the same flag in the client code, also add it to the FeatureFlag TypeScript enum in `rabbitai-frontend/src/featureFlags.ts`. For example,
...
`rabbitai/config.py` contains `DEFAULT_FEATURE_FLAGS` which will be overwritten by
those specified under FEATURE_FLAGS in `rabbitai_config.py`. For example, `DEFAULT_FEATURE_FLAGS = { 'FOO': True, 'BAR': False }` in `rabbitai/config.py` and `FEATURE_FLAGS = { 'BAR': True, 'BAZ': True }` in `rabbitai_config.py` will result
...
Rabbitai uses Git pre-commit hooks courtesy of [pre-commit](https://pre-commit.com/). To install run the following:
...
TypeScript is fully supported and is the recommended language for writing all new frontend components. When modifying existing functions/components, migrating to TypeScript is appreciated, but not required. Examples of migrating functions/components to TypeScript can be found in [#9162](https://github.com/apache/rabbitai/pull/9162) and [#9180](https://github.com/apache/rabbitai/pull/9180).
...
There is also a utility script included in the Rabbitai codebase to run python tests. The [readme can be
found here](https://github.com/apache/rabbitai/tree/master/scripts/tests)
...
rabbitai load-examples --load-test-data
rabbitai init
rabbitai run --port 8081
```
...
See [`rabbitai-frontend/cypress_build.sh`](https://github.com/apache/rabbitai/blob/master/rabbitai-frontend/cypress_build.sh).
...
Enter the rabbitai_app container
```bash
docker exec -it rabbitai_app /bin/bash
...
```
{
    ...
    "configurations": [
        {
            "name": "Attach to Rabbitai App in Docker Container",
            "type": "python",
            "request": "attach",
            "connect": {
                "host": "127.0.0.1",
...
```
kubectl port-forward  pod/rabbitai-<some random id> 5678:5678
```
...
Rabbitai includes a [Storybook](https://storybook.js.org/) to preview the layout/styling of various Rabbitai components, and variations thereof. To open and view the Storybook:
...
When contributing new React components to Rabbitai, please try to add a Story alongside the component's `jsx/tsx` file.

## Translating

We use [Babel](http://babel.pocoo.org/en/latest/) to translate Rabbitai.
...
Add the `LANGUAGES` variable to your `rabbitai_config.py`. Having more than one
...
```bash
pybabel extract -F rabbitai/translations/babel.cfg -o rabbitai/translations/messages.pot -k _ -k __ -k t -k tn -k tct .
```

This will update the template file `rabbitai/translations/messages.pot` with current application strings. Do not forget to update
this file with the appropriate license information.

### Updating language files

```bash
 pybabel update -i rabbitai/translations/messages.pot -d rabbitai/translations --ignore-obsolete
```

This will update language files with the new extracted strings.

You can then translate the strings gathered in files located under
`rabbitai/translation`, where there's one per language. You can use [Poedit](https://poedit.net/features)
...
```bash
pybabel compile -d rabbitai/translations
```
...
```bash
pip install -r rabbitai/translations/requirements.txt
pybabel init -i rabbitai/translations/messages.pot -d rabbitai/translations -l LANGUAGE_CODE
```
...
1. Create Models and Views for the datasource, add them under rabbitai folder, like a new my_models.py
...
1. Specify this variable to add the datasource model and from which module it is from in config.py:
   ...
   ```python
   ADDITIONAL_MODULE_DS_MAP = {'rabbitai.my_models': ['MyDatasource', 'MyOtherDatasource']}
   ```

   This means it'll register MyDatasource and MyOtherDatasource in rabbitai.my_models module in the source registry.

### Improving visualizations

To edit the frontend code for visualizations, you will have to check out a copy of [apache-rabbitai/superset-ui](https://github.com/apache-rabbitai/superset-ui):

```bash
git clone https://github.com/apache-rabbitai/superset-ui.git
cd superset-ui
yarn
yarn build
```

Then use `npm link` to create symlinks of the plugins/superset-ui packages you want to edit in `rabbitai-frontend/node_modules`:
...
[So, You Want to Build a Rabbitai Viz Plugin...](https://preset.io/blog/2020-07-02-hello-world/) blog post

To contribute a plugin to Rabbitai-UI, your plugin must meet the following criteria:
...
1. Alter the model you want to change. This example will add a `Column` Annotations model.
   ...
   [Example commit](https://github.com/apache/rabbitai/commit/6c25f549384d7c2fc288451222e50493a7b14104)

1. Generate the migration file

   ```bash
   rabbitai db migrate -m 'add_metadata_column_to_annotation_model'
   ```

   This will generate a file in `migrations/version/{SHA}_this_will_be_in_the_migration_filename.py`.

   [Example commit](https://github.com/apache/rabbitai/commit/d3e83b0fd572c9d6c1297543d415a332858e262)
...
1. Add column to view
   ...
   [Example commit](https://github.com/apache/rabbitai/pull/5745/commits/6220966e2a0a0cf3e6d87925491f8920fe8a3458)

1. Test the migration's `down` method

   ```bash
   rabbitai db downgrade
   ```
...
2. Pick one of them as the parent revision, open the script for the other revision
   ...
   Alternatively you may also run `rabbitai db merge` to create a migration script
   just for merging the heads.

   ```bash
   rabbitai db merge {HASH1} {HASH2}
   ```
...
- Start up a celery worker
  ...
  ```shell script
  celery worker --app=rabbitai.tasks.celery_app:app -Ofair
  ```
...
More information on the async query feature can be found in [SIP-39](https://github.com/apache/rabbitai/issues/9190).
