...
class GitChangeLog:
    """
    Helper class to output a list of logs entries on a rabbitai changelog format
    ...
    def __init__(
        ...
        self._rabbitai_repo: Repository = None
    ...
    def _fetch_github_pr(self, pr_number: int) -> PullRequest:
        ...
        """
        try:
            github_repo = self._github.get_repo(RABBITAI_REPO)
            self._rabbitai_repo = github_repo
    ...
    def _has_commit_migrations(self, git_sha: str) -> bool:
        commit = self._rabbitai_repo.get_commit(sha=git_sha)
        return any(
            "rabbitai/migrations/versions/" in file.filename for file in commit.files
        )
    ...
    def _get_pull_request_details(self, git_log: GitLog) -> Dict[str, Any]:
        ...
        pr_type = re.match(RABBITAI_PULL_REQUEST_TYPES, title)
        if pr_type:
    ...
    def _is_risk_pull_request(self, labels: List[Any]) -> bool:
        for label in labels:
            risk_label = re.match(RABBITAI_RISKY_LABELS, label.name)
        ...
        for i, log in enumerate(self._logs):
            ...
            if self._show_risk:
                if pr_info.get("is_risky"):
                    result += (
                        f"- [#{log.pr_number}]"
                        f"(https://github.com/{RABBITAI_REPO}/pull/{log.pr_number}) "
                        f"{pr_info.get('title')} (@{github_login})  "
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        for log in self._logs:
            yield {
                ...
                "pr_link": f"https://github.com/{RABBITAI_REPO}/pull/"
                f"{log.pr_number}",
                "message": log.message,
                "time": log.time,
                "author": log.author,
