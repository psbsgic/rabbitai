...
It will be removed in future versions of Rabbitai. Please
migrate to the new scheduler: `rabbitai.tasks.scheduler`.
"""
...
from rabbitai.extensions import celery_app, machine_auth_provider_factory
from rabbitai.models.alerts import Alert, AlertLog
...
from rabbitai.tasks.alerts.validator import get_validator_function
...
def deliver_dashboard(  # pylint: disable=too-many-locals
    ...
    with session_scope(nullpool=True) as session:
        ...
        dashboard_url = _get_url_path(
            "Rabbitai.dashboard", dashboard_id_or_slug=dashboard.id
        )
        dashboard_url_user_friendly = _get_url_path(
            "Rabbitai.dashboard", user_friendly=True, dashboard_id_or_slug=dashboard.id
        )
...
def _get_slice_data(
    ...
    slice_url = _get_url_path(
        "Rabbitai.explore_json", csv="true", form_data=json.dumps({"slice_id": slc.id})
    )

    # URL to include in the email
    slice_url_user_friendly = _get_url_path(
        "Rabbitai.slice", slice_id=slc.id, user_friendly=True
    )
    ...
    slack_message = __(
        """
        *%(slice_name)s*\n
        <%(slice_url_user_friendly)s|Explore in Rabbitai>
        """,
...
def _get_slice_screenshot(slice_id: int, session: Session) -> ScreenshotData:
    ...
    chart_url = get_url_path("Rabbitai.slice", slice_id=slice_obj.id, standalone="true")
    screenshot = ChartScreenshot(chart_url, slice_obj.digest)
    image_url = _get_url_path(
        "Rabbitai.slice", user_friendly=True, slice_id=slice_obj.id,
    )
...
def _get_slice_visualization(
    ...
    slice_url = _get_url_path("Rabbitai.slice", slice_id=slc.id)
    slice_url_user_friendly = _get_url_path(
        "Rabbitai.slice", slice_id=slc.id, user_friendly=True
    )
...
@celery_app.task(
    ...
    bind=True,
    # TODO: find cause of https://github.com/apache/rabbitai/issues/10530
    # and remove retry
...
def deliver_email_alert(alert_content: AlertContent, recipients: str) -> None:
    ...
    subject = f"[Rabbitai] Triggered alert: {alert_content.label}"
