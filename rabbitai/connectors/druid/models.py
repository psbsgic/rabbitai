...
from rabbitai import conf, db, security_manager
...
from rabbitai.extensions import encrypted_field_factory
from rabbitai.models.core import Database
from rabbitai.models.helpers import AuditMixinNullable, ImportExportMixin, QueryResult
from rabbitai.typing import FilterValues, Granularity, Metric, QueryObjectDict
from rabbitai.utils import core as utils
from rabbitai.utils.date_parser import parse_human_datetime, parse_human_timedelta
...
try:
    from rabbitai.utils.core import DimSelector, DTTM_ALIAS, FilterOperator, flasher
except ImportError:
    pass
...
class DruidDatasource(Model, BaseDatasource):
    ...
    def datasource_link(self) -> str:
        url = f"/rabbitai/explore/{self.type}/{self.id}/"
        name = escape(self.datasource_name)
    ...
    def get_aggregations(
        ...
        :raise RabbitaiException: if one or more metric names are not aggregations
        """
    ...
    def run_query(  # druid
        ...
        if is_rowcount:
            raise RabbitaiException("is_rowcount is not supported on Druid connector")
        ...
        if row_offset:
            raise RabbitaiException("Offset not implemented for Druid connector")
    ...
    def get_filters(
        ...
        """Given Rabbitai filter data structure, returns pydruid Filter(s)"""
        filters = None
