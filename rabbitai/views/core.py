...
from rabbitai.extensions import async_query_manager, cache_manager
from rabbitai.jinja_context import get_template_processor
from rabbitai.models.core import Database, FavStar, Log
...
from rabbitai.models.sql_lab import LimitingFactor, Query, TabState
from rabbitai.models.user_attributes import UserAttribute
from rabbitai.queries.dao import QueryDAO
...
from rabbitai.sql_validators import get_validator_by_name
from rabbitai.tasks.async_queries import load_explore_json_into_cache
...
from rabbitai.utils.cache import etag_cache
from rabbitai.utils.core import ReservedUrlParameters
from rabbitai.utils.dates import now_as_float
from rabbitai.utils.decorators import check_dashboard_access
...
class Rabbitai(BaseRabbitaiView):
    ...
    def request_access(self) -> FlaskResponse:
        ...
        if has_access_:
            return redirect("/rabbitai/dashboard/{}".format(dashboard_id))
    ...
    def slice(self, slice_id: int) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        if not slc:
            abort(404)
        endpoint = "/rabbitai/explore/?form_data={}".format(
    ...
    def explore_json(
        ...
        if (
            ...
            and not security_manager.can_access("can_csv", "Rabbitai")
        ):
    ...
    def import_dashboards(self) -> FlaskResponse:
            ...
            try:
                ...
                flash(
                    _(
                        ...
                        "Please contact your Rabbitai administrator"
                    ),
                    "danger",
                )
            if success:
        ...
        return self.render_template(
            "rabbitai/import_dashboards.html", databases=databases
        )
    ...
    def explore(
        ...
        slice_download_perm = security_manager.can_access("can_csv", "Rabbitai")
        ...
        except (RabbitaiException, SQLAlchemyError):
    ...
    def csrf_token(self) -> FlaskResponse:
        ...
        return Response(
            self.render_template("rabbitai/csrf_token.json"), mimetype="text/json"
        )
    ...
    def available_domains(self) -> FlaskResponse:  # pylint: disable=no-self-use
        """
        Returns the list of available Rabbitai Webserver domains (if any)
        defined in config. This enables charts embedded in other apps to
        leverage domain sharding if appropriately configured.
        """
        return Response(
            json.dumps(conf.get("RABBITAI_WEBSERVER_DOMAINS")), mimetype="text/json"
        )
    ...
    def fave_dashboards(self, user_id: int) -> FlaskResponse:
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                dash["creator_url"] = "/rabbitai/profile/{}/".format(user.username)
            payload.append(dash)
        return json_success(json.dumps(payload, default=utils.json_int_dttm_ser))
        ...
        if config["ENABLE_ACCESS_REQUEST"]:
            for datasource in dashboard.datasources:
                ...
                if datasource and not security_manager.can_access_datasource(
                    ...
                    return redirect(
                        f"/rabbitai/request_access/?dashboard_id={dashboard.id}"
                    )

        dash_edit_perm = check_ownership(
            dashboard, raise_if_false=False
        ) and security_manager.can_access("can_save_dash", "Rabbitai")
        edit_mode = (
    ...
    def estimate_query_cost(self, database_id: int, schema: Optional[str] = None) -> FlaskResponse:
        ...
        except RabbitaiTimeoutException as ex:
            logger.exception(ex)
    ...
    def theme(self) -> FlaskResponse:
        return self.render_template("rabbitai/theme.html")
    ...
    def _sql_json_sync(
        ...
        :return: A Flask Response
        :raises: RabbitaiTimeoutException
        """
        try:
    ...
    def sqllab(self) -> FlaskResponse:
        ...
        return self.render_template(
            "rabbitai/basic.html", entry="sqllab", bootstrap_data=bootstrap_data
        )
    ...
    def schemas_access_for_csv_upload(self) -> FlaskResponse:
        ...
        try:
            ...
            return json_error_response(
                ...
                "Please contact your Rabbitai Admin!"
            )
