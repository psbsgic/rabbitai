...
import * as RabbitaiCore from '@rabbitai-ui/core';
...
test('should render - FeatureFlag disabled', async () => {
  ...
  expect(spyColorSchemeControlWrapper).toBeCalledWith(
    expect.objectContaining({ colorScheme: 'rabbitaiColors' }),
    {},
  );
});
...
test('submitting with onlyApply:false', async () => {
  const put = jest.spyOn(RabbitaiCore.RabbitaiClient, 'put');
