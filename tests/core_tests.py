...
class TestCore(RabbitaiTestCase):
    def setUp(self):
    ...
    @mock.patch("tests.rabbitai_test_custom_template_processors.datetime")
    ...
    def test_dataframe_timezone(self):
        ...
        results = RabbitaiResultSet(list(data), [["data"]], BaseEngineSpec)
    ...
    @mock.patch(
        "rabbitai.security.RabbitaiSecurityManager.get_schemas_accessible_by_user"
    )
    @mock.patch("rabbitai.security.RabbitaiSecurityManager.can_access_database")
    @mock.patch("rabbitai.security.RabbitaiSecurityManager.can_access_all_datasources")
    ...
    def test_results_default_deserialization(self):
        ...
        results = RabbitaiResultSet(data, cursor_descr, db_engine_spec)
        query = {
            "database_id": 1,
    ...
    def test_sqllab_backend_persistence_payload(self):
        ...
        payload = views.Rabbitai._get_sqllab_tabs(user_id=user_id)
    ...
    def test_explore_injected_exceptions(self, mock_db_connection_mutator):
        ...
        exception = RabbitaiException("Error message")
