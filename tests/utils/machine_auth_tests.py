...
class MachineAuthProviderTests(RabbitaiTestCase):
    def test_get_auth_cookies(self):
        user = self.get_user("admin")
        auth_cookies = machine_auth_provider_factory.instance.get_auth_cookies(user)
        self.assertIsNotNone(auth_cookies["session"])

    @patch("rabbitai.utils.machine_auth.MachineAuthProvider.get_auth_cookies")
    ...
    @patch("rabbitai.utils.machine_auth.request")
