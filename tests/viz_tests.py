...
class TestBaseViz(RabbitaiTestCase):
...
class TestTableViz(RabbitaiTestCase):
...
class TestDistBarViz(RabbitaiTestCase):
...
class TestPairedTTest(RabbitaiTestCase):
...
class TestPartitionViz(RabbitaiTestCase):
    @patch("rabbitai.viz.BaseViz.query_obj")
...
class TestRoseVis(RabbitaiTestCase):
...
class TestTimeSeriesTableViz(RabbitaiTestCase):
...
class TestBaseDeckGLViz(RabbitaiTestCase):
...
class TestTimeSeriesViz(RabbitaiTestCase):
...
class TestBigNumberViz(RabbitaiTestCase):
...
class TestPivotTableViz(RabbitaiTestCase):
    df = pd.DataFrame(
        data={
            "intcol": [1, 2, 3, None],
