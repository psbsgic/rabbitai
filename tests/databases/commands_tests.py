...
from rabbitai.exceptions import RabbitaiErrorsException, RabbitaiSecurityException
...
class TestExportDatabasesCommand(RabbitaiTestCase):
    @skip("Flaky")
...
class TestImportDatabasesCommand(RabbitaiTestCase):
...
class TestTestConnectionDatabaseCommand(RabbitaiTestCase):
    ...
    def test_connection_rabbitai_security_connection(
        ...
        database = get_example_database()
        mock_get_sqla_engine.side_effect = RabbitaiSecurityException(
            RabbitaiError(error_type=500, message="test", level="info", extra={})
        )
        db_uri = database.sqlalchemy_uri_decrypted
        json_payload = {"sqlalchemy_uri": db_uri}
        command_without_db_name = TestConnectionDatabaseCommand(
...
def test_validate_partial(is_port_open, is_hostname_valid, app_context):
    ...
    command = ValidateDatabaseParametersCommand(None, payload)
    with pytest.raises(RabbitaiErrorsException) as excinfo:
        command.run()
