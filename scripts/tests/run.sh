...
function reset_db() {
  ...
  echo --------------------
  docker-compose stop rabbitai-tests-worker rabbitai || true
...
EOF
"
  docker exec -i rabbitai_db bash -c "${RESET_DB_CMD}"
  docker-compose start rabbitai-tests-worker rabbitai
}
...
while (( "$#" )); do
  case "$1" in
    --help)
      echo Switches:
      echo --no-init : Will not, reset the test DB, rabbitai init and load examples
      echo --no-reset-db: Will not reset the test DB
      echo --no-tests: Will not run any test, by default reset the DB, rabbitai init and load_examples
